...
class SetUI(object):
    ...
    def download_music(self):
        ...
        :return:
        """
        if not os.path.exists(r'E:\文件下载\Music\spider'):
            os.mkdir(r"E:\文件下载\Music\spider")
        if self.song_num is not None:
            song_name = self.song_name.replace('/', '-').replace('\\', '-') + '--' + self.song_author + ".mp3"
            try:
                save_path = os.path.join(r'E:\文件下载\Music\spider\{}'.format(song_name))

                true_path = os.path.abspath(save_path)
                resp = requests.get(self.song_url)
                with open(true_path, 'wb') as file:
                    file.write(resp.content)
                    mes_box.showinfo(title='下载成功', message=r'歌曲：%s,保存地址为%s' % (self.song_name, true_path))
            except Exception:
